...
enum IQTESTER_TYPE
{
	...
	IQTYPE_XSTREAM_5G                   /*!< IQxstream-5G tester */
};
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_AnalyzeCWMaxPowSegm(double intervalLength = 0.0,
                                     double captureLength = 0.0);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_EnableRfPort(int rfPort1Enabled=1, int rfPort2Enabled=1, int rfPort3Enabled=1);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_SetAp(IQLINK_BAND apBand, IQLINK_TECH_MODE apMode, IQLINK_BANDWIDTH apBandwidth, int apChannel, int apMcsIndex, int apNumStream, double apEndpointPower=15, bool encryptionOn = false, IQLINK_ENCRYPT_TYPE encryptionType = WPA2, bool aggregateMpduOn = true, IQLINK_HT_PPDU_FORMAT htGreenfield = LINK_AP_MIXED, IQLINK_GUARD_INTERVAL guardInterval = LINK_GI_LONG, IQLINK_OFDM_CODING ofdmCoding = LINK_BCC, double attenuation = 50.0, bool beaconFixedRate = false);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_Tx_Power_Run(double attenuationLevel, double testTimeDuration=5, int psduLength=1090, IQLINK_DGEN_MODE dgenMode=LINK_FPING, double triggerLevelRf1=-15, double triggerLevelRf2=-15, double triggerLevelRf3=-15);
